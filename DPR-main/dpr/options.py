...
def setup_cfg_gpu(cfg):
    """
    Setup params for CUDA, MPS, CPU & distributed training
    """
    ...
    elif cfg.local_rank == -1 or cfg.no_cuda:  # single-node multi-gpu, mps, or cpu mode
        if torch.cuda.is_available() and not cfg.no_cuda:
            device = str(torch.device("cuda"))
            cfg.n_gpu = torch.cuda.device_count()
        elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available() and not cfg.no_cuda:
