...
def generate_question_vectors(
    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            # Some tensorizers return 1D token tensors, while Wav2vec paths may
            # return 2D tensors. Support both for local retrieval.
            max_vector_len = max(q_t.size(1) if q_t.dim() > 1 else q_t.size(0) for q_t in batch_tensors)
            min_vector_len = min(q_t.size(1) if q_t.dim() > 1 else q_t.size(0) for q_t in batch_tensors)
...
def save_results(
    ...
    for i, q in enumerate(questions):
        ...
        if question_ids:
            results_item["question_id"] = question_ids[i]
